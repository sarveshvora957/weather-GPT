  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-x-hidden text-slate-100">
      ...
      <Navbar
        ...
        onOpenSearch={() => setSearchModalOpen(false)}
      />
      ...
      <div className="flex-1 flex max-w-7xl w-full mx-auto">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 overflow-y-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                High-resolution 48-hour and 14-day numerical simulation models for{" "}
          ...
          {/* Hourly Scroller */}
          {loading ? <HourlySkeleton /> : <HourlyForecast items={hourly} unit={unit} />}

          {/* Meteorological Dynamic Charts */}
          {loading ? <ChartSkeleton /> : <WeatherCharts hourly={hourly} unit={unit} />}

          {/* 14-Day Extended Daily Cards */}
          {loading ? <HourlySkeleton /> : <DailyForecast items={daily} unit={unit} />}
        </main>
      </div>
